/*************************************************
 * Sorted_single_list_tester<Type>
...
#ifndef SORTED_SINGLE_LIST_TESTER_H
#define SORTED_SINGLE_LIST_TESTER_H
...
#include "Exception.h"
#include "Tester.h"
#include "Sorted_single_list.h"
#include "Single_node_tester.h"

#include <iostream>

template <typename Type>
class Sorted_single_list_tester:public Tester< Sorted_single_list<Type> > {
	using Tester< Sorted_single_list<Type> >::object;
	using Tester< Sorted_single_list<Type> >::command;

	public:
		Sorted_single_list_tester( Sorted_single_list<Type> *obj = nullptr ):Tester< Sorted_single_list<Type> >( obj ) {
			// empty
		}
 ...
 *   head            head            the head is not 'nullptr' and starts a Single_node_tester on that
 *                                   node until the 'exit' command is typed
 *   head0           head            the head is 'nullptr'
 *   tail            tail            the tail is not 'nullptr' and starts a Single_node_tester on that
 *                                   node until the 'exit' command is typed
 ...
 *  Mutators
 *
 *   insert n        insert          the element can be inserted into the sorted list (always succeeds)
...
template <typename Type>
void Sorted_single_list_tester<Type>::process() {
	if ( command == "new" ) {
		object = new Sorted_single_list<Type>();
	...
	} /*else if ( command == "push_front" ) {//COMMNETED OUT BY SOHEIL
		...
		object->push_back( n );
		std::cout << "Okay" << std::endl;
	}*/ else if (command == "insert") {//COMMNETED OUT BY SOHEIL
	// push the next object read to the front of the linked list
	...
	std::cout << "Okay" << std::endl;
		...
		Sorted_single_list<Type> *new_object = new Sorted_single_list<Type>();

		*new_object = *object;

		std::cout << "Okay" << std::endl;

		Sorted_single_list_tester<Type> tester( new_object );
