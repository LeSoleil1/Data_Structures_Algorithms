/*************************************************
 * Cyclic_double_list_tester
 * A class for testing cyclic doubly linked lists.
 *
...
#ifndef CYCLIC_DOUBLE_LIST_TESTER_H
#define CYCLIC_DOUBLE_LIST_TESTER_H
...
class Cyclic_double_list_tester:public Tester< Cyclic_double_list<Type> > {
	using Tester< Cyclic_double_list<Type> >::object;
	using Tester< Cyclic_double_list<Type> >::command;

	public:
		Cyclic_double_list_tester( Cyclic_double_list<Type> *obj = nullptr ):Tester< Cyclic_double_list<Type> >( obj ) {
			// empty
		}
 ...
 * void process()
 *
 * Process the current command.  For cyclic doubly linked lists, these include:
 *
 *  Accessors
 *
 ...
 *   head            head            the head is not 0 and sends the next N instructions to Double_node_tester
...
template <typename Type>
void Cyclic_double_list_tester<Type>::process() {
	if ( command == "new" ) {
		object = new Cyclic_double_list<Type>();
		...
		// checks if the first integer in the linked list equals the next integer read
		...
		// checks if the last integer in the linked list equals the next integer read
		...
		// checks that the head is not null, and if it is not,
		// the next integer gives the number of commands which should
		...
		// check if the next integer read in is in the linked list
		...
		// push the next integer read to the front of the linked list
		...
		// push the next integer read to the back of the linked list
		...
		// pop the first integer from the linked list
		...
		// pop the last integer from the linked list
		...
		Cyclic_double_list<Type> *new_object = new Cyclic_double_list<Type>();

		*new_object = *object;

		std::cout << "Okay" << std::endl;

		Cyclic_double_list_tester tester( new_object );
